#define AVERAGECOUNT 12
#define MAX_BOUNCE 12

//--scene data---------------------------------------------------------------------
#define SPHERECOUNT 6
//xyz = pos, w = radius
const vec4 AllSpheres[SPHERECOUNT]=vec4[SPHERECOUNT](
    vec4(0.0,0.0,0.0,2.0),//sphere A
    vec4(0.0,0.0,-1.0,2.0),//sphere B
    vec4(0.0,-1002.0,0.0,1000.0),//ground
    vec4(0.0,0.0,+1002,1000.0),//back wall
    vec4(-1004.0,0.0,0.0,1000.0),//left wall    
    vec4(+1004.0,0.0,0.0,1000.0)//right wall
);

// Raymarching Functions
vec3 algebraicRay(vec3 rd, float distance, float a, float b, float c) {
    // Polynomial equation: ax^2 + bx + c
    float polynomial = a * distance * distance + b * distance + c;
    polynomial = distance*(a+b+c)*(a-b+c);//+(sin(iTime));
    return rd + vec3(0.0, polynomial, polynomial);
}
vec3 test1(vec3 originalDir, float distance) {
    // Parameters for the sinusoidal path
    float amplitude = length(originalDir); // Amplitude of the sinusoid
    float frequency = abs(sin(iTime*.05))+.901; // Frequency of the sinusoid

    // Calculate the sinusoidal offset
    float sineOffset = amplitude * tan(distance * frequency);

    // Modify the original direction with the offset
    // This is a simplified example; the actual implementation
    // would depend on how you want the sinusoid to affect the path
    vec3 modifiedDir = originalDir + vec3(sineOffset, 0.0, 0.0);
    
    return normalize(modifiedDir);
}
vec3 test2(vec3 originalDir, float distance) {
    // Parameters for the sinusoidal path
   // float amplitude = .15; // Amplitude of the sinusoid
    float frequency = (sin(iTime*.25))+.141; // Frequency of the sinusoid
    float amplitude = 2.*abs(sin(length(originalDir))); // Amplitude of the sinusoid

    // Calculate the sinusoidal offset
    
    float sineOffsetx = (amplitude/10.) * sin(distance*distance);
        float sineOffsety = (amplitude/10.) * sin(-distance* distance);
            float sineOffsetz = (amplitude/10.) * cos(-distance*distance);

    
    vec3 modifiedDir;
    // Modify the original direction with the offset
    // This is a simplified example; the actual implementation
    // would depend on how you want the sinusoid to affect the path
      modifiedDir = originalDir+vec3(sineOffsetx, sineOffsety, sineOffsetz);
       vec3 extraDir = vec3(mix(originalDir,vec3(sineOffsetx, sineOffsety, sineOffsetz),.5));

    return normalize(modifiedDir);
}
vec3 modifyRayDirection(vec3 originalDir, float distance,float a,float b, float c) {
    // Fourier series parameters
    float baseFrequency = abs(sin(iTime * 0.05)) + 0.251; // Base frequency
    baseFrequency = .5;
    // Initialize the offset
    vec3 offset = vec3(0.0);

    // Summation of multiple sinusoids
    const int terms = 17; // Number of terms in the Fourier series
    for (int n = 1; n <= terms; n++) {
        // Amplitude varies with 'n'
       // float amplitude = 0.015 / float(n);
    float amplitude = (1./float(n))*1.;//((length(originalDir))); // Amplitude of the sinusoid

        // Harmonic frequencies
        float frequency = float(n) * baseFrequency;

        // Sinusoidal term
        float sineTermx = amplitude * sin(distance * frequency*sqrt(abs(distance)));
        float sineTermy = amplitude * (sin(distance*frequency));
        float sineTermz = amplitude * sin(distance * frequency*.1/sqrt(abs(distance)));

        // Adding the term to the offset
        offset += vec3(0., 0., sineTermy);
    }
    float polynomial = a * distance * distance + b * distance + c;
   float polynomiala = fract((a*b*c)/(1.+a-b-c))*sin(iTime/5.);
   float polynomialb = a*a+b-c;
    polynomiala=polynomialb-sin(distance);
   polynomiala=sin(a+b+c)*(a*b*c);

    // Modify the original direction with the cumulative offset
    vec3 modifiedDir =  originalDir+ offset*sin(sin(polynomialb+polynomiala));

    return normalize(modifiedDir);
}
// Raymarching Functions

vec3 test3(vec3 originalDir, float distance,float a,float b,float c) {

    // Fourier series parameters
    float baseFrequency = abs(sin(iTime * 0.05)) + 0.251; // Base frequency
    baseFrequency = .5;
    // Initialize the offset
    vec3 offset = vec3(0.0);

    // Summation of multiple sinusoids
    const int terms = 17; // Number of terms in the Fourier series
    for (int n = 1; n <= terms; n++) {
        // Amplitude varies with 'n'
       // float amplitude = 0.015 / float(n);
    float amplitude = (1./float(n))*1.;//((length(originalDir))); // Amplitude of the sinusoid

        // Harmonic frequencies
        float frequency = float(n) * baseFrequency;

        // Sinusoidal term
        float sineTermx = amplitude * sin(distance * frequency*sqrt(abs(distance)));
        float sineTermy = amplitude * (sin(distance*frequency));
        float sineTermz = amplitude * sin(distance * frequency*.1/sqrt(abs(distance)));

        // Adding the term to the offset
        offset += vec3(0., 0., sineTermy);
    }
    offset = vec3(distance*originalDir.x*originalDir.x*originalDir.x-distance*originalDir.y*originalDir.y+distance*originalDir.z);
  // Polynomial equation: ax^2 + bx + c
    float polynomial = a * distance * distance + b * distance + c;
   float polynomiala = fract((a*b*c)/(1.+a-b-c))*sin(iTime/5.);
   float polynomialb = a*a+b-c;
    polynomiala=polynomialb-sin(distance);
   polynomiala=sin(a+b+c)*(a*b*c);
    //polynomial = distance*tan(length(originalDir));//(a-b+c);//+(sin(iTime));
   // polynomial=polynomial*polynomial;
     return originalDir+  vec3(polynomiala, (polynomialb), polynomiala);
    //return originalDir;
    // Modify the original direction with the cumulative offset
   // vec3 modifiedDir = originalDir + offset;

  //  return normalize(modifiedDir);
}
//-----------------------------------------------------------------------
float raySphereIntersect(vec3 r0, vec3 rd, vec3 s0, float sr) {
    // - r0: ray origin
    // - rd: normalized ray direction
    // - s0: sphere center
    // - sr: sphere radius
    // - Returns distance from r0 to first intersecion with sphere,
    //   or -1.0 if no intersection.
    float a = dot(rd, rd);
    vec3 s0_r0 = r0 - s0;
    float b = 2.0 * dot(rd, s0_r0);
    float c = dot(s0_r0, s0_r0) - (sr * sr);
    if (b*b - 4.0*a*c < 0.0) {
        return -1.0;
    }
    return (-b - sqrt((b*b) - 4.0*a*c))/(2.0*a);
}
//-----------------------------------------------------------------------
struct HitData
{
    float rayLength;
    vec3 normal;
};
HitData AllObjectsRayTest(vec3 rayPos, vec3 rayDir)
{
    HitData hitData;
    hitData.rayLength = 9999.0; //default value if can't hit anything

    for(int i = 0; i < SPHERECOUNT; i++)
    {
        vec3 sphereCenter = AllSpheres[i].xyz;
        float sphereRadius = AllSpheres[i].w;
        //----hardcode sphere pos animations-------------------------------------
        if(i == 0)
        {
            float t = fract(iTime * 0.7);
            t = -4.0 * t * t + 4.0 * t;
            sphereCenter.y += t * 0.7;
            
            sphereCenter.x += sin(iTime) * 2.0;
            sphereCenter.z += cos(iTime) * 2.0;
        }
             
        if(i == 1)
        {
            float t = fract(iTime*0.47);
            t = -4.0 * t * t + 4.0 * t;
            sphereCenter.y += t * 1.7;
            
            sphereCenter.x += sin(iTime+3.14) * 2.0;
            sphereCenter.z += cos(iTime+3.14) * 2.0;
        }             
        //---------------------------------------
                
        float resultRayLength = raySphereIntersect(rayPos,rayDir,sphereCenter,sphereRadius);
        if(resultRayLength < hitData.rayLength && resultRayLength > 0.1)
        {
            //if a shorter(better) hit ray found, update
            hitData.rayLength = resultRayLength;
            vec3 hitPos = rayPos + rayDir * resultRayLength;
    		hitData.normal = normalize(hitPos - sphereCenter);
        }
    }
    
    //all test finished, return shortest(best) hit data
    return hitData;
}
//--random functions-------------------------------------------------------------------
float rand01(float seed) { return fract(sin(seed)*43758.5453123); }
vec3 randomInsideUnitSphere(vec3 rayDir,vec3 rayPos, float extraSeed)
{
    return vec3(rand01(iTime * (rayDir.x + rayPos.x + 0.357) * extraSeed),
                rand01(iTime * (rayDir.y + rayPos.y + 16.35647) *extraSeed),
                rand01(iTime * (rayDir.z + rayPos.z + 425.357) * extraSeed));
}
//---------------------------------------------------------------------
vec4 calculateFinalColor(vec3 cameraPos, vec3 cameraRayDir, float AAIndex)
{
    //init
    vec3 finalColor = vec3(0.0);
    float absorbMul = 1.0;
    vec3 rayStartPos = cameraPos;
    vec3 rayDir = cameraRayDir;
    
    //only for CineShader, to show depth
    float firstHitRayLength = -1.0;
    
    //can't write recursive function in GLSL, so write it in a for loop
    //will loop until hitting any light source / bounces too many times
    for(int i = 1; i < MAX_BOUNCE; i++)
    {
        
        HitData h = AllObjectsRayTest(rayStartPos + rayDir * 0.0001,rayDir);//+0.0001 to prevent ray already hit at start pos
        rayDir = normalize(modifyRayDirection(rayDir, h.rayLength,rayDir.x*rayDir.y,rayDir.y*rayDir.z,rayDir.z*rayDir.x));
        //only for CineShader, to show depth
        firstHitRayLength = firstHitRayLength < 0.0 ? h.rayLength : firstHitRayLength;
               
        //if ray can't hit anything, rayLength will remain default value 9999.0
        //which enters this if()
        //** 99999 is too large for mobile, use 9900 as threshold now **
        if(h.rayLength >= 9900.0)
        {
            vec3 skyColor = vec3(0.7,0.85,1.0);//hit nothing = hit sky color
            finalColor = skyColor * absorbMul;
            break;
        }   
               
		absorbMul *= 0.698; //every bounce absorb some light(more bounces = darker)
        
        //update rayStartPos for next bounce
		rayStartPos = rayStartPos + rayDir * h.rayLength; 
        //update rayDir for next bounce
        float rougness = 1.-abs(sin(h.rayLength)); //hardcode "drag the window LR to control roughness"
		rayDir += normalize(reflect(rayDir,h.normal) + randomInsideUnitSphere(rayDir,rayStartPos,AAIndex) * rougness);       
    }
    
    return vec4(finalColor,firstHitRayLength);//alpha nly for CineShader, to show depth
}
//-----------------------------------------------------------------------
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    // Normalized pixel coordinates (from 0 to 1)
    vec2 uv = fragCoord/iResolution.xy;
    
	uv = uv * 2.0 - 1.0;//transform from [0,1] to [-1,1]
    uv.x *= iResolution.x / iResolution.y; //aspect fix

    vec3 cameraPos = vec3(sin(iTime * 0.47) * 4.0,sin(iTime * 0.7)*8.0+6.0,-25.0);//camera pos animation
    vec3 cameraFocusPoint = vec3(0,0.0 + sin(iTime),0);//camera look target point animation
    vec3 cameraDir = normalize(cameraFocusPoint - cameraPos);
    
    //TEMPCODE: fov & all ray init dir, it is wrong!!!!
    //----------------------------------------------------
    float fovTempMul = 0.2 + sin(iTime * 0.4) * 0.05;//fov animation
    vec3 rayDir = normalize(cameraDir + vec3(uv,0) * fovTempMul);
    //----------------------------------------------------

    vec4 finalColor = vec4(0);
    for(int i = 1; i <= AVERAGECOUNT; i++)
    {
        finalColor+= calculateFinalColor(cameraPos,rayDir, float(i));
    }
    finalColor = finalColor/float(AVERAGECOUNT);//brute force AA & denoise
    finalColor.rgb = pow(finalColor.rgb,vec3(1.0/2.2));//gamma correction
    
    //only for CineShader, to show depth
    float z = finalColor.w; //z is linear world space distance from camera to surface
    float cineShaderZ; //expect 0~1
    cineShaderZ = pow(clamp(1.0 - max(0.0,z-21.0) * (1.0/6.0),0.0,1.0),2.0);
    
    //result
    fragColor = vec4(finalColor.rgb,cineShaderZ);
}
