// A Fork of "Bidirectional path tracing" by @reindernijhoff in 2014 that 
// experiments with adding additional structure to the LightPathNode such as velocity/acceleration
...
void initLightSphere( float time ) {
	lightSphere = vec4( 3.0+2.*sin(time*2.),2.8+2.*sin(time*0.9),3.0+4.*cos(time*0.7),0.5);
    	//lightSphere = vec4( 3.,1.8,-2.,.5);
...
void constructLightPath(inout float seed) {
    ...
    ro = lightSphere.xyz + ro;
    vec3 color = LIGHTCOLOR;
    float t = iTime;
    //vec3 prevpos = getpos( lpNodes[0].position);
        //vec3 prevvel = getvel( lpNodes[0].velocity);

    lpNodes[0].position = ro;
    lpNodes[0].color = color;
    lpNodes[0].normal = rd;
    lpNodes[0].velocity = vec3(0.);
    lpNodes[0].acc = vec3(0.);
    
    for( int i=1; i<LIGHTPATHLENGTH; ++i ) {
        lpNodes[i].position = lpNodes[i].color = lpNodes[i].normal =lpNodes[i].velocity =lpNodes[i].acc =  vec3(1.) ;
    }
    ...
    for( int i=1; i<LIGHTPATHLENGTH; i++ ) {
        ...
        if( res.y > -0.5 && res.y < 4. ) {
           ...
           // lpNodes[i].velocity = (lpNodes[i].position - lpNodes[i-1].position)/(1.1*float(i));
            //lpNodes[i].acc = (lpNodes[i].velocity - lpNodes[i-1].velocity )/(1.1*float(i));
            //lpNodes[i].velocity +=lpNodes[i].acc;
            //lpNodes[i].position += lpNodes[i].velocity;
           // ro = lpNodes[i].position;
            //rd = lpNodes[i].velocity;
                        //lpNodes[i].normal = rd;
...
vec3 traceEyePath( in vec3 ro, in vec3 rd, inout float seed ) {
    ...
    for( int j=0; j<EYEPATHLENGTH; ++j ) {
        ...
        //rd*=lpNodes[j].velocity;
          //ro=lpNodes[j].position;
               //ro-=lpNodes[j].acc;
	    ...
	    for( int i=1; i<LIGHTPATHLENGTH-1; ++i ) {
            if( i+j >= MAXPATHLENGTH-1 ) continue;
                          if (mod(float(i),2.)!=178.)
                          {
                         // lpNodes[i].position.x-= (float(i)*10.);
                        // lpNodes[i].position.z-= (float(i)*12.);
                         // lpNodes[i].position.y-= (float(i)*10.);
                          ...
                          {
                          // lpNodes[i].position.x+= (float(i)*10.);
                         // lpNodes[i].position.z+= (float(i)*100.);
                         // lpNodes[i].position.y+= (float(i)*10.);
                           }


                    lpNodes[i].velocity = (lpNodes[i].position - lpNodes[i-1].position)/(12.0001);
            lpNodes[i].acc = accel+ (lpNodes[i].velocity - lpNodes[i-1].velocity )/(12.0001);
           // lpNodes[i].acc =vec3(0.0010);
            lpNodes[i+1].position *= 1.+ (lpNodes[i].velocity*lpNodes[i].acc);
                        //lpNodes[i+1].position *= 1.+ (lpNodes[i].acc*accel);

           // lpNodes[i].velocity +=  (lpNodes[i].acc*1.00);
            //lpNodes[i].position = normalize(lpNodes[i].velocity);
          // lpNodes[i].position+= lpNodes[i].velocity;
                     //ro+= (1.+length(lpNodes[i].acc))*rd+length(lpNodes[i].velocity)*lpNodes[i].position;
                        lpNodes[i].normal = reflect(-rd,normal);

                        //ro+=lpNodes[i].velocity;
                        //ro+=lpNodes[i].acc;

            vec3 lp = ((  lpNodes[i-1].position+ lpNodes[i].position+lpNodes[i+1].position)/3.);
            vec3 lpn = normalize(  lp );
            vec3 lc = average( lpNodes[i-1].color.rgb,lpNodes[i].color.rgb,lpNodes[i+1].color.rgb);
